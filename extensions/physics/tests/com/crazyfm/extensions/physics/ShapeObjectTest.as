...
package com.crazyfm.extensions.physics
{
	...
	public class ShapeObjectTest
	{
		...
		{
			Assert.assertNotNull(s.vertices);
			Assert.assertEquals(s.vertices[0].x, 0);
			Assert.assertEquals(s.vertices[0].y, 0);
			Assert.assertEquals(s.vertices[1].x, 10);
			Assert.assertEquals(s.vertices[1].y, 10);
			Assert.assertEquals(s.vertices[2].x, 50);
			Assert.assertEquals(s.vertices[2].y, 75);
		}
	}
}
